...
def criar_filme(filme: dict, r = Depends(get_redis_client)):
    ...
    return resposta_sucesso("Filme inserido com sucesso no Redis")

@router.post("/atores")
@tratar_erros
def criar_ator(ator: dict, r = Depends(get_redis_client)):
    inserir_ator(r, ator)
    return resposta_sucesso("Ator inserido com sucesso no Redis")

@router.post("/elenco")
@tratar_erros
def criar_elenco(relacao: dict, r = Depends(get_redis_client)):
    inserir_elenco(r, relacao)
    return resposta_sucesso("Elenco inserido com sucesso no Redis")
...
def filmes_por_genero(genero: str, r = Depends(get_redis_client)):
    ...
    return resposta_sucesso("Consulta de filmes por gênero realizada com sucesso", {"filmes": filmes})
...
def atualizar_nota(titulo_id: str, nova_nota: float, r = Depends(get_redis_client)):
    ...
    return resposta_sucesso("Nota do filme atualizada com sucesso")
...
def deletar_filme(titulo_id: str, r = Depends(get_redis_client)):
    ...
    return resposta_sucesso("Filme removido com sucesso do Redis")
...
def contar_filmes(r = Depends(get_redis_client)):
    contagem = contar_filmes_por_ano(r)
    return resposta_sucesso("Contagem de filmes por ano realizada com sucesso", {"contagem_por_ano": contagem})

@router.get("/filmes/media-nota/genero")
@tratar_erros
def media_por_genero(r = Depends(get_redis_client)):
    media = media_notas_por_genero(r)
    return resposta_sucesso("Média de notas por gênero calculada com sucesso", {"media_notas_por_genero": media})
